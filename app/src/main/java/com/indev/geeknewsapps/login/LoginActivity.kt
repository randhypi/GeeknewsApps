...
class LoginActivity : AppCompatActivity() {
    ...
    private fun resetPassword() {
        ...
        resetPassword.btn_submit.setOnClickListener {
            ...
            if (email.isEmpty()) {
                resetPassword.et_emailResetPass.error = getString(R.string.fill_text)
                resetPassword.et_emailResetPass.requestFocus()
                return@setOnClickListener
            }

            if (!Patterns.EMAIL_ADDRESS.matcher(email).matches()) {
                resetPassword.et_emailResetPass.error = getString(R.string.invalid_email)
                resetPassword.et_emailResetPass.requestFocus()
                return@setOnClickListener
            }
            ...
            FirebaseAuth.getInstance().sendPasswordResetEmail(email).addOnCompleteListener {
                if (it.isSuccessful) {
                    Toast.makeText(
                            applicationContext,
                            "Lihat email untuk link lupa password",
                            Toast.LENGTH_LONG
                    ).show()
                } else {
    ...
    private fun signInWithEmail() {
        ...
        if (email.isEmpty()) {
            et_emailAddress.error = resources.getString(R.string.fill_text)
            et_emailAddress.requestFocus()
            return
        }

        if (!Patterns.EMAIL_ADDRESS.matcher(email).matches()) {
            et_emailAddress.error = "Kesalahan email"
            et_emailAddress.requestFocus()
            return
        }

        if (password.isEmpty()) {
            et_password.error = resources.getString(R.string.fill_text)
